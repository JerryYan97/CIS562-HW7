...
void AParticleSystem::drawOpenGL(vec3 mCameraPos)
{
    for (unsigned int i = 0; i < mParticles.size(); i++)
    {
		...
		if (particle->isAlive())
		{
			...
			vec3 particlePos = vec3(pos[0], pos[1], pos[2]);
			//vec3 desiredZVector = mCameraPos;
			//desiredZVector = desiredZVector.Normalize();
			//mat3 m = mat3(vec3(0, 0, desiredZVector[0]), vec3(0, 0, desiredZVector[1]), vec3(0, 0, desiredZVector[2]));
			//std::cout << "m:" << std::endl << m << std::endl;
			//vec3 axis;
			//double angle;
			//m.ToAxisAngle(axis, angle);
			//std::cout << "camera backward" << mCameraPos << std::endl;
